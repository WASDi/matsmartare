...
grep __PRELOADED_STATE__ index.html | perl -pe 's|.*<script>window.__PRELOADED_STATE__=(.*?),window.__.*|\1|' | python3 -m json.tool > preloaded.json

cd jsons/latest
curl 'https://api.matsmart.se/api/v1.0/routes?market=SE' > routes.json
curl 'https://api.matsmart.se/api/product-displays?market=SE' > products.json
