...
main = do
    ...
    case parseItems before_products before_routes of
        ...
        Right before_items -> case parseItems after_products after_routes of
...
insertPriceChangesInDb priceChanges = do
    ...
    stmt <- prepare c "INSERT INTO price_changes (item_id, price_before, price_after, created) VALUES (?, ?, ?, current_timestamp);"
...
insertPriceChange stmt priceChange = execute stmt [toSql (item_id priceChange), toSql (before priceChange), toSql (after priceChange)]
